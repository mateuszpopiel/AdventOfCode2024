...
auto is_cell_on_the_right_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.y + 1 < map[current_cell.x].size() &&
         map[current_cell.x][current_cell.y + 1] == region_type;
}

auto is_cell_below_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.x + 1 < map.size() &&
         map[current_cell.x + 1][current_cell.y] == region_type;
}

auto is_cell_on_the_left_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.y != 0 &&
         map[current_cell.x][current_cell.y - 1] == region_type;
}

auto is_cell_above_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.x != 0 &&
         map[current_cell.x - 1][current_cell.y] == region_type;
}

auto is_cell_on_upper_left_diagonal_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.x != 0 && current_cell.y != 0 &&
         map[current_cell.x - 1][current_cell.y - 1] == region_type;
}

auto is_cell_on_upper_right_diagonal_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.x != 0 && current_cell.y + 1 < map[current_cell.x].size() &&
         map[current_cell.x - 1][current_cell.y + 1] == region_type;
}

auto is_cell_on_lower_left_diagonal_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.x + 1 < map.size() && current_cell.y != 0 &&
         map[current_cell.x + 1][current_cell.y - 1] == region_type;
}

auto is_cell_on_lower_right_diagonal_in_region(
  const Map &map, const Coordinates current_cell, const char region_type) -> bool {
  return current_cell.x + 1 < map.size() && current_cell.y + 1 < map[current_cell.x].size() &&
         map[current_cell.x + 1][current_cell.y + 1] == region_type;
}

auto was_cell_checked(
  const std::vector<Coordinates> &checked_cells, const Coordinates cell) -> bool {
  return std::any_of(checked_cells.begin(), checked_cells.end(), [cell](const Coordinates &checked_cell) {
    return checked_cell == cell;
  });
}

auto get_cell_perimeter(
  const Map &map, const Coordinates current_cell, const char region_type, std::vector<Coordinates> &checked_cells) -> size_t {
  size_t perimeter = 0;
...
auto direction_changes(const Map &map, const Coordinates cell) -> size_t {
  ...
  if (unknown_cell_on_the_right && unknown_cell_below) { changes++; }
  if (unknown_cell_below && unknown_cell_on_the_left) { changes++; }
  if (unknown_cell_on_the_left && unknown_cell_above) { changes++; }
  if (unknown_cell_above && unknown_cell_on_the_right) { changes++; }
  if (!unknown_cell_on_the_left && !unknown_cell_above && unknown_cell_on_upper_left_diagonal) { changes++; }
  if (!unknown_cell_on_the_right && !unknown_cell_above && unknown_cell_on_upper_right_diagonal) { changes++; }
  if (!unknown_cell_on_the_left && !unknown_cell_below && unknown_cell_on_lower_left_diagonal) { changes++; }
  if (!unknown_cell_on_the_right && !unknown_cell_below && unknown_cell_on_lower_right_diagonal) { changes++; }
...
auto get_fence_price(const Map &map, const bool part_2) -> size_t {
  ...
  for (auto region_beginning = find_region_beginning(modifiable_map);
       region_beginning.has_value();
    ...
    if (!part_2) { fence_price += (region.perimeter * region.area); }
    else { fence_price += (get_num_of_vertices(map, region_cells) * region.area); }
  }
  return fence_price;
}
